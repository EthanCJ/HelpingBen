...
public class PpmAsciiReader {
	...
	private void readPixels(List<String> lines) {
		...
		while(it.hasNext()){
			String[] colorValues=it.next().trim().split("\\s+");;
			for(int x=0;x<colorValues.length;x+=3){
				int red;
				int green;
				int blue;
				//Read RGB Values
				try{
					red=Integer.parseInt(colorValues[x]);
					green=Integer.parseInt(colorValues[x+1]);
					blue=Integer.parseInt(colorValues[x+2]);
				}catch(Exception e){
				...
				if(red>maxValue || red<0){
					throw new PpmException("Invalid Pixel Input");
				}
				...
				//Set pixel color
				img.set(row,column,new Color(red,green,blue));
				column+=1;
			}
			row+=1;
			column=0;
